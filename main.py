...
def evaluate(model, sample):
    ...
    sample = np.expand_dims(sample[:,:,None][:,:,[0]], axis=0) # Prepare sample for prediction 
    # Get prediction
...
def loop(cap, model, classes):
    ...
    while(True):
        ...
        frame[0:preview_size, 0:preview_size, :] = cv2.resize(np.copy(sample), (preview_size,preview_size), interpolation = cv2.INTER_AREA)[:,:,None][:,:,[0,0,0]]*255
        # Draw detection box
        cv2.rectangle(frame, (width//2-detection_box_size, height//2-detection_box_size), (width//2+detection_box_size,height//2+detection_box_size), color, 2)
        # Add Text
        frame = putText(frame, f"{pred_prob*100.0:.1f}% {classes[pred_idx]}", (width//2-detection_box_size,height//2+detection_box_size), color, 20)
        text = f"Objetive: {classes[target_idx]}\n\n"
        if help_enabled: text = text + "Controls:\n- C: To close/open controls\n- A/D: Previous/Next class\n- R: Random Class\n- W/S: Increase/Decrease Detection Box Size\n- +/-: Increase/Decrease Window Size\n- P: Pause\n- S: Screenshot (only on Pause)\n- Q: Quit"
        frame = putText(frame, text, (10, preview_size+10), color, 18)
        # Resize frame
        frame = cv2.resize(frame, None, fx=win_scale, fy=win_scale, interpolation = cv2.INTER_CUBIC)
        ...
        if opt == ord('p'): # Pause
            while(True):
                if cv2.waitKey(2) & 0xFF == ord('s'): cv2.imwrite("frame.png", frame) # Take screenshot
                else: break

        if opt == ord('w') and detection_box_size < height: # Increase detection box size
...
if __name__ == "__main__":
    ...
    p.add_argument("-c", "--classes", required=True, 
                   help="Classes names file path (.txt)")
    p.add_argument("-cap", "--camera-id", type=int, default=0,
